...
float Calz = 0;
int speedSpinL = 100 , speedSpinR = 100;
...
void SpinL() {
  ...
  if (batt >= 8.35) {
    // ประมาณ 8.4V
    speedSpin = 120;
    limitAngle = 31;//เลี้ยวเกินให้เพิ่มค่า เลี้ยวขาดให้ลดค่า
  }
  else if (batt >= 8.15) {
    // ประมาณ 8.2 - 8.3V
    speedSpin = 100;
    limitAngle = 20;//เลี้ยวเกินให้เพิ่มค่า เลี้ยวขาดให้ลดค่า
  }
  else if (batt >= 7.95) {
    // ประมาณ 8.0 - 8.1V
    speedSpin = 130;
...
void SpinR() {
  ...
  if (batt >= 8.35) {
    // ประมาณ 8.4V
    speedSpin = 100;
    limitAngle = 70;//เลี้ยวเกินให้เพิ่มค่า เลี้ยวขาดให้ลดค่า
  }
  else if (batt >= 8.15) {
    // ประมาณ 8.2 - 8.3V
    speedSpin = 115;
    limitAngle = 50;//เลี้ยวเกินให้เพิ่มค่า เลี้ยวขาดให้ลดค่า
  }
  else if (batt >= 7.95) {
    // ประมาณ 8.0 - 8.1V
    speedSpin = 100;
...
void SpinL(int slow) {
  ...
  while (1) {
    ...
    if (error < -180) error += 360;
    if (abs(error) <= limitAngle) break;
    int targetSpeedL = constrain(abs(error) * 2, 60, speedSpinL);
    int targetSpeedR = constrain(abs(error) * 2, 60, speedSpinR);
    MT(-targetSpeedL, targetSpeedR);
  }
...
void ControlSpeedForward(int DesiredAngle, float TargetInput) {
  ...
  float speedKp = 0.08, speedKi = 0.06, speedKd = 0.01;
